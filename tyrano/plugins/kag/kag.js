tyrano.plugin.kag = {
  ...
  init_game: function () {
    ...
    if (typeof that.variable.sf._system_config_se_volume !==
      "undefined") that.config["defaultSeVolume"] = String(that.variable.sf._system_config_se_volume);
    ...
    if (that.variable.sf._system_config_already_read_text_color) that.config["alreadyReadTextColor"] =
    ...
    this.tyrano.base.setBaseSize(this.config.scWidth,
      this.config.scHeight);
    ...
    if (this.config.WordBreak == "false") j_message_inner.css("word-break",
      "break-all");
    ...
    for (var i = 1; i < num_message_layer; i++) {
      ...
      this.layer.appendObj(message_layer_name,
        "fore", j_message1);
    ...
    if (this.kag.config.defaultShadow == "true") this.stat.default_font.shadow =
      $.convertColor(this.kag.config.defaultShadowColor);
  ...
  pushBackLog: function (str, type) {
    ...
    if (type == "join") {
      ...
      if (index >= 0) {
        ...
        this.variable.tf["system"]["backlog"][this.variable.tf.system.backlog.length -
          1
        ] = tmp + str
  ...
  loadScenario: function (file_name,
    call_back) {
    var that = this;
  ...
  preloadAll: function (storage, callbk) {
    ...
    if (typeof storage == "object" && storage.length >= 0) {
      ...
      var sum = 0;
      for (var i =
          0; i < storage.length; i++) that.kag.preload(storage[i], function () {
        sum++;
