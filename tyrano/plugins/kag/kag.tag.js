tyrano.plugin.kag.ftag = {
  ...
  convertEntity: function (pm) {
    ...
    for (key in pm) {
      ...
      if (val.length > 0 && c === "&") pm[key] = this.kag.embScript(val.substr(1,
        val.length));
  ...
  showMessage: function (message_str, pm) {
    ...
    (function (jtext) {
      ...
      var pchar = function (pchar) {
        ...
        current_str += c;
        if (that.kag.stat.is_skip != true && that.kag.stat.is_nowait != true && ch_speed > 3) that.kag.appendMessage(jtext,
          current_str);
        if (index <= _message_str.length) {
          that.kag.stat.is_adding_text = true;
          if (that.kag.stat.is_click_text == true || that.kag.stat.is_skip == true || that.kag.stat.is_nowait == true) pchar(pchar);
  ...
  start: function (pm) {
    ...
    if (pm.layer != "base") {
      ...
      if (pm.pos != "") switch (pm.pos) {
        ...
        case "l":
          pm.left =
            this.kag.config["scPositionX.left"];
          break;
        case "left_center":
        case "lc":
    ...
    (function () {
      ...
      j_button.click(function (event) {
        ...
        if (_pm.role != "") {
          ...
          if (_pm.role == "save" || _pm.role == "menu" || _pm.role == "quicksave" || _pm.role ==
            "sleepgame")
